...
Destructuring imports from ramda *does not necessarily prevent importing the entire library*, discussed [here](https://github.com/scabbiaza/ramda-webpack-tree-shaking-examples). You can manually cherry-pick methods like the following, which would only grab the parts necessary for `identity` to work:
